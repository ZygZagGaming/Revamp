...
public class EnderBookCopyRecipe extends CustomRecipe {
    ...
    public boolean matches(CraftingContainer container, Level world) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (loop != ItemStack.EMPTY) {
                if (loop.is(Registry.ItemRegistry.ENDER_BOOK_ITEM.get())) {
    ...
    public ItemStack assemble(CraftingContainer container) {
        ...
        if (!tag.contains("id")) return ItemStack.EMPTY;
            ...
            ItemStack toReturn = Registry.ItemRegistry.ENDER_BOOK_ITEM.get().getDefaultInstance();
    ...
    public RecipeSerializer<?> getSerializer() {
        return Registry.RecipeSerializerRegistry.ENDER_BOOK_COPY_CRAFTING.get();
    }
}
