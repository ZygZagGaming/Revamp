...
public class IridiumHoeItem extends HoeItem implements ISocketable {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> text, TooltipFlag flag) {
        ...
        if (s != Socket.NONE && world != null) {
            ...
            m.append(Component.translatable( str + "_ability.revamp.hoe." + socket.name().toLowerCase()).withStyle(ChatFormatting.GOLD));
            text.add(m);
            text.add(Component.translatable("description." + str + "_ability.revamp.hoe." + socket.name().toLowerCase()));
            if (hasCooldown()) {
