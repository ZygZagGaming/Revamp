...
public class IridiumChestplateItem extends ArmorItem implements ISocketable {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> text, TooltipFlag flag) {
        ...
        if (s != Socket.NONE) {
            ...
            m.append(Component.translatable("passive_ability.revamp.chestplate." + socket.name().toLowerCase()).withStyle(ChatFormatting.GOLD));
            text.add(m);
            text.add(Component.translatable("description.passive_ability.revamp.chestplate." + socket.name().toLowerCase()));
            if (hasCooldown()) {
